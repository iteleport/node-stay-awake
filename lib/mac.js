...
module.exports.prevent = function(cb) {
    ...
    // Create an assertion to declare that user is active. If the display is off, this option turns the display on and
    // prevents the display from going into idle sleep. If a timeout is not specified with '-t' option, then this assertion
    // is taken with a default of 5 second timeout.
    // Pass a timeout of 3 seconds.
    child_process.execFile('caffeinate', ['-u', '-t3']);

    // just for compatibility with windows.js till we fix this properly
